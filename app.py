...
initial_prompt = f"""you are a chatbot named Sensei, and your work is to answer the questions based on our company trusty money and its works,
Read the following paragraph: {pre_fed_data}\n\n"""
...
def chatbot():
    ...
    query_prompt = initial_prompt + f"""
    Do not mention about the presence of this paragraph in the response of my further questions.
    If something in the query is not mentioned in the paragraph, only say you don't know about it and nothing else.
    Now response to this query: {query}. Explain the answer a little."""
    
    response = chat_session.send_message(query_prompt)
    return jsonify({"response": f"{response.candidates[0].content.parts[0].text}"})
